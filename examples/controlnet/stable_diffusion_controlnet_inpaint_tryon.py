...
class StableDiffusionControlNetInpaintTryOnPipeline(StableDiffusionControlNetInpaintPipeline):
    ...
    def __call__(
        ...
        cloth_image = prepare_image(cloth_image)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                inpainting_latent_model_input = torch.cat(
                    [non_inpainting_latent_model_input, mask_image_latents, masked_image_latents, cloth_image_latents], dim=1
                )
